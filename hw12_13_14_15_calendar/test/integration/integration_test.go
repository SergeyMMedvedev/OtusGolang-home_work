...
func TestCreateEventProduceAndConsume(t *testing.T) {
	fmt.Println("Wait all services up...")
	...
	require.NoError(t, err)
	dayEvent := listDayResponse.GetEventList()[len(listDayResponse.GetEventList())-1]
	...
	require.NoError(t, err)
	weekEvent := listWeekEvents.GetEventList()[len(listWeekEvents.GetEventList())-1]
	...
	cmd := exec.Command("cat", "/tmp/delivered.txt")
	...
	cmd = exec.Command("rm", "/tmp/delivered.txt")
	_, err = cmd.Output()
	require.NoError(t, err)
}
